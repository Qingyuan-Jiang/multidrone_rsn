...
class VPDroneSimCV:
    ...
    def move_to(self, camera_pose, time_sleep, k):
        """
        ...
        """
        self.pos_drones_sim[k] = torch.from_numpy(camera_pose.position.to_numpy_array().astype(np.float16)).float()
        self.ori_drones_sim[k] = torch.from_numpy(camera_pose.orientation.to_numpy_array().astype(np.float16)).float()

        print("--------------------------------")
        print("Moving to pos. \t", self.pos_drones_sim[k])
        print("Moving to ori. \t", self.ori_drones_sim[k])

        pos_actor_sim_w, ori_actor_sim_w_quat = self.pos_drones_sim[k].numpy(), self.ori_drones_sim[k].numpy()
        ori_actor_sim_w_matrix = R.from_quat(ori_actor_sim_w_quat).as_matrix()

        pos_actor_sim_d = (self.rot_const @ pos_actor_sim_w.reshape(3, -1)).reshape(-1, 3)
        ori_actor_sim_d_quat = R.from_matrix(self.rot_const @ ori_actor_sim_w_matrix).as_quat()
        # ori_actor_sim_d_quat = R.from_matrix(ori_actor_sim_w_matrix).as_quat()

        # cpose_msg = cpose2msg(pos_actor_sim_d.astype(np.float16), ori_actor_sim_d_quat.astype(np.float16))
        cpose_msg = cpose2msg(pos_actor_sim_d, ori_actor_sim_d_quat)
        self.c.simSetVehiclePose(cpose_msg, True)
        # self.c.simSetVehiclePose(camera_pose, True, 'drone_' + str(k + 1))
